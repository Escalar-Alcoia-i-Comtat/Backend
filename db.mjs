    ...
    try {
        ...
        for (let i in queries) {
            ...
            const query = `SELECT ${fieldsToSelect}
                           FROM escalaralcoiaicomtat.${table}
                           WHERE ${whereParameter}${limitParameter}`;
            log("📝 SQL query: ", query);
 ...
 */
export const queryData = async (table, ...fields) => {
    let conn, res;
    try {
        conn = await pool.getConnection();
        const query = `SELECT ${fields.length > 0 ? fields.join(',') : '*'}
                       FROM escalaralcoiaicomtat.${table}`;
        log("📝 SQL query: ", query);
 ...
 * @return {Promise<any>} The result of the request.
 */
export const queryWhere = async (table, params, ...fields) => {
    let conn, res;
    let wheres = [];
    for (let k in params)
    ...
    try {
        ...
        const query = `SELECT ${selector}
                       FROM escalaralcoiaicomtat.${table} ${whereParam}`;
        log("📝 SQL query: " + query);
        res = await conn.query(query);
    } catch (err) {
        throw err;
 ...
 */
export const update = async (table, selector, data) => {
    let conn, res;
    try {
        conn = await pool.getConnection();
        const updates = [];
        ...
        const query = `UPDATE escalaralcoiaicomtat.${table}
                       SET ${updates.join(', ')}
...
export const insert = async (table, data) => {
    ...
    try {
        conn = await pool.getConnection();
        const query = `INSERT INTO escalaralcoiaicomtat.${table} (${keys.join(', ')})
                       values (${values.join(', ')})`;
        log("📝 SQL query: " + query);
        res = await conn.query(query);
    } catch (err) {
